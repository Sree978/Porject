
1. Code + Dockerfile

2. Code Scan : CQA (Code Quality Analysis) ---> Sonar

3. Build Maven (mvn clean package) (cp -r target Docker-app)

4. Artifact : Nexus

5. Dockerfiles ---> Build ---> image (docker build -t appimage Docker-app) &&
   (docker build -t dbimage Docker-db)

6. Image Scan ---> (trivy image appimage) && (trivy image dbimage)

7. Registry --->

   a. docker tag dbimage shaikmustafa/manual:db
   b. docker tag appimage shaikmustafa/manual:app
   c. docker push shaikmustafa/manual:db
   d. docker push shaikmustafa/manual:app

8. Deploy ---> Pre-Prod & Prod

9. Post Build Actions ---> Pipeline status

Take four server  
1) manager
2) jenkins
3) worker 1
4) worker 2

 1 ) Install docker yum install docker -y && systemctl start docker
  in manager , worker1 & worker 2
2 ) run docker swarm init over manager & paste code in worker 1 & 2
3) docker compose  install in manger 

cmd => 
curl -L "https://github.com/docker/compose/releases/download/1.29.2/docker-compose-$(uname -s)-$(uname -m)" -o /usr/local/bin/docker-compose

sudo chmod +x /usr/local/bin/docker-compose

sudo ln -s /usr/local/bin/docker-compose /usr/bin/docker-compose

sudo yum install python-pip

sudo pip3 install docker-compose

docker-compose --version

take a jenkins server 
 install jenkins 

 we have to take source code to manager  pipeline needs to run in manager 
 will use master and slave concept
 manage jenkins > nodes > 
 

